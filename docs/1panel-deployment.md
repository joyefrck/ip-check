# 1Panel 部署指南

本指南将帮助你在 1Panel 面板上部署 IP-Check 项目。

## 前置要求

- ✅ 已安装 1Panel 面板
- ✅ 服务器已安装 Docker 和 Docker Compose
- ✅ 有域名（可选，用于反向代理）

## 部署方式

### 方式一：使用 1Panel 应用商店（推荐新手）

如果项目已发布到 1Panel 应用商店，可以直接安装。

### 方式二：使用 Docker Compose（推荐）

#### 步骤 1：从 GitHub 拉取项目

**方法 A：使用 1Panel 终端（推荐）**

1. 登录 1Panel 面板
2. 进入 **主机** → **终端**
3. 执行以下命令：

```bash
# 进入应用目录
cd /opt/1panel/apps/

# 从 GitHub 克隆项目
git clone https://github.com/joyefrck/ip-check.git ip-check

# 进入项目目录
cd ip-check
```

**方法 B：使用 SSH 连接服务器**

```bash
# SSH 连接到服务器
ssh root@你的服务器IP

# 克隆项目
cd /opt/1panel/apps/
git clone https://github.com/joyefrck/ip-check.git ip-check
cd ip-check
```

> 💡 **提示**：如果是私有仓库，需要先配置 GitHub 访问令牌或 SSH 密钥。

#### 步骤 2：配置环境变量

1. 在项目目录中创建 `.env` 文件：

```bash
cp .env.example .env
```

2. 编辑 `.env` 文件（可选）：

```env
PORT=3000
NODE_ENV=production
```

#### 步骤 3：在 1Panel 中创建容器

**方法 A：使用 1Panel 的容器管理**

1. 进入 **容器** → **编排**
2. 点击 **创建编排**
3. 填写信息：
   - **名称**：`ip-check`
   - **路径**：选择项目目录（如 `/opt/1panel/apps/ip-check`）
   - **docker-compose.yml**：系统会自动识别
4. 点击 **确认** 创建
5. 点击 **启动** 按钮启动容器

**方法 B：使用终端命令**

1. 进入 **主机** → **终端**
2. 执行以下命令：

```bash
cd /opt/1panel/apps/ip-check
docker-compose up -d
```

#### 步骤 4：验证部署

1. 进入 **容器** → **容器**
2. 查看 `ip-check` 容器状态，应该显示 **运行中**
3. 点击容器名称，查看日志确认启动成功
4. 访问 `http://服务器IP:3000` 测试

---

### 方式三：配置反向代理（推荐生产环境）

如果你想通过域名访问（如 `https://ip.yourdomain.com`），需要配置反向代理。

#### 使用 1Panel 的 OpenResty/Nginx

1. 进入 **网站** → **网站**
2. 点击 **创建网站** → **反向代理**
3. 填写配置：
   - **域名**：`ip.yourdomain.com`
   - **代理地址**：`http://127.0.0.1:3000`
   - **启用 HTTPS**：开启（推荐）
   - **自动申请证书**：开启（使用 Let's Encrypt）
4. 点击 **确认** 创建

#### Nginx 配置示例

如果手动配置，参考以下配置：

```nginx
server {
    listen 80;
    server_name ip.yourdomain.com;
    
    # 重定向到 HTTPS
    return 301 https://$server_name$request_uri;
}

server {
    listen 443 ssl http2;
    server_name ip.yourdomain.com;
    
    # SSL 证书配置
    ssl_certificate /path/to/cert.pem;
    ssl_certificate_key /path/to/key.pem;
    
    # 反向代理配置
    location / {
        proxy_pass http://127.0.0.1:3000;
        proxy_http_version 1.1;
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection 'upgrade';
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
        proxy_cache_bypass $http_upgrade;
    }
}
```

---

## 常见操作

### 查看容器日志

**在 1Panel 中：**
1. 进入 **容器** → **容器**
2. 点击 `ip-check` 容器
3. 点击 **日志** 标签

**使用命令行：**
```bash
docker logs -f ip-check
```

### 重启容器

**在 1Panel 中：**
1. 进入 **容器** → **容器**
2. 选择 `ip-check` 容器
3. 点击 **重启** 按钮

**使用命令行：**
```bash
cd /opt/1panel/apps/ip-check
docker-compose restart
```

### 停止容器

**在 1Panel 中：**
1. 进入 **容器** → **容器**
2. 选择 `ip-check` 容器
3. 点击 **停止** 按钮

**使用命令行：**
```bash
cd /opt/1panel/apps/ip-check
docker-compose down
```

### 更新应用

**从 GitHub 拉取最新代码并重新部署：**

```bash
# 进入项目目录
cd /opt/1panel/apps/ip-check

# 拉取最新代码
git pull origin main  # 或 git pull origin master，根据你的分支名

# 停止旧容器
docker-compose down

# 重新构建并启动
docker-compose up -d --build
```

**在 1Panel 中操作：**

1. 进入 **主机** → **终端**
2. 执行上述命令
3. 或在 **容器** → **编排** 中点击 **重建** 按钮

### 修改端口

1. 编辑 `.env` 文件，修改 `PORT` 值
2. 重启容器使配置生效

---

## 防火墙配置

如果无法访问，需要在 1Panel 中开放端口：

1. 进入 **主机** → **防火墙**
2. 点击 **添加规则**
3. 填写信息：
   - **端口**：`3000`（或你配置的端口）
   - **协议**：`TCP`
   - **策略**：`允许`
4. 点击 **确认**

**注意**：如果使用反向代理，只需开放 80 和 443 端口，不需要开放 3000 端口。

---

## 性能优化建议

### 1. 使用反向代理缓存

在 Nginx 配置中添加缓存：

```nginx
proxy_cache_path /var/cache/nginx levels=1:2 keys_zone=ip_check_cache:10m max_size=1g inactive=60m;

location / {
    proxy_cache ip_check_cache;
    proxy_cache_valid 200 10m;
    proxy_cache_key "$scheme$request_method$host$request_uri";
    # ... 其他配置
}
```

### 2. 启用 Gzip 压缩

```nginx
gzip on;
gzip_vary on;
gzip_proxied any;
gzip_comp_level 6;
gzip_types text/plain text/css text/xml text/javascript application/json application/javascript application/xml+rss;
```

### 3. 配置资源限制

编辑 `docker-compose.yml`，添加资源限制：

```yaml
services:
  ip-check:
    # ... 其他配置
    deploy:
      resources:
        limits:
          cpus: '1'
          memory: 512M
        reservations:
          cpus: '0.5'
          memory: 256M
```

---

## 故障排查

### 容器无法启动

1. 查看容器日志：`docker logs ip-check`
2. 检查端口是否被占用：`netstat -tulpn | grep 3000`
3. 检查 Docker 服务状态：`systemctl status docker`

### 无法访问应用

1. 检查容器是否运行：`docker ps | grep ip-check`
2. 检查防火墙规则
3. 检查反向代理配置
4. 测试本地访问：`curl http://localhost:3000`

### 内存不足

1. 增加服务器内存
2. 或在 docker-compose.yml 中限制内存使用
3. 优化 Next.js 构建配置

---

## 安全建议

1. ✅ 使用 HTTPS（通过反向代理配置 SSL）
2. ✅ 定期更新 Docker 镜像
3. ✅ 使用防火墙限制访问
4. ✅ 定期备份数据
5. ✅ 监控容器资源使用情况

---

## 监控和日志

### 使用 1Panel 的监控功能

1. 进入 **监控** 菜单
2. 查看容器 CPU、内存使用情况
3. 设置告警规则

### 日志管理

建议配置日志轮转，避免日志文件过大：

```yaml
services:
  ip-check:
    # ... 其他配置
    logging:
      driver: "json-file"
      options:
        max-size: "10m"
        max-file: "3"
```

---

## 总结

使用 1Panel 部署 IP-Check 项目的推荐流程：

1. 上传项目文件到服务器
2. 使用 1Panel 的容器编排功能启动
3. 配置反向代理和 SSL 证书
4. 开放必要的防火墙端口
5. 访问域名测试

如有问题，请查看容器日志或参考故障排查部分。
